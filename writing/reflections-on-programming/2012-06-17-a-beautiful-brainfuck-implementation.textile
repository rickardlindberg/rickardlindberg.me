...
The most interesting thing to note is that we use a "map":http://hackage.haskell.org/packages/archive/containers/0.5.0.0/doc/html/Data-Map-Lazy.html instead of a list structure. This makes it easy to create an infinite tape. If we ask for a value at a position that does not yet exist, we just return the default value 0.
...
The function that converts a Brainfuck program to a list of tokens uses the "Parsec":https://hackage.haskell.org/package/parsec library and looks like this:
