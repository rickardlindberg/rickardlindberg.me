...
First of all we need a way to create a data tape. Next we need a way to
...
`bfTokens` is our parser. It parses a list of `bfToken`. `bfToken`
...
There is one special trick going on here, and that is in the last case
