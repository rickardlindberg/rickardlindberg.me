...
This is equivalent to using the `--compile` option with the argument `-` which
stands for stdin:
...
The `--copy` option of the RLMeta compiler can be used to copy this main file
as is.

Combining these pieces into a single compile command looks like this:
...
* The first part, `--embed SUPPORT src/support.py`, tells the compiler to
  generate a Python variable named `SUPPORT` containing the contents of the
  file `src/support.py`. The `--embed` option is the last option to the compiler
  that we have not yet seen. The RLMeta compiler needs the support library in a
  variable so that it can later generate it with the `--support` option.
* Next, the `--support` options tells the compiler to generate the support
  library that is embedded in it.
* The `--compile ...` section tells the compiler to compile the given grammar
  files.
