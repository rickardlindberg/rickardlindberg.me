...
class Parser(Grammar):
    ...
    def assemble(self, I, LABEL):
        ...
        I('CALL', 'maybeAction')
        I('BIND', 'ys')
        I('ACTION', lambda scope: concat([splice(0, 'Scope'), splice(0, concat([splice(0, 'And'), splice(1, scope.lookup('xs')), splice(1, scope.lookup('ys'))]))]))
        I('POP_SCOPE')
        I('RETURN')
        LABEL('expr')
        I('BACKTRACK', 8)
        ...
        I('LIST_END')
        I('BIND', 'xs')
        I('ACTION', lambda scope: concat([splice(0, concat([splice(0, 'Action'), splice(1, scope.lookup('x')), splice(2, scope.lookup('xs'))]))]))
        I('POP_SCOPE')
        I('COMMIT', 37)
        LABEL(36)
        I('PUSH_SCOPE')
        ...
        I('CALL', 'space')
        I('MATCH_OBJECT', ')')
        I('ACTION', lambda scope: concat([splice(0, 'Call'), splice(0, scope.lookup('x')), splice(1, scope.lookup('ys'))]))
        I('POP_SCOPE')
        I('COMMIT', 53)
        LABEL(52)
        I('PUSH_SCOPE')
...
class CodeGenerator(Grammar):
    ...
    def assemble(self, I, LABEL):
        ...
        I('CALL', 'ast')
        I('BIND', 'x')
        I('ACTION', lambda scope: join(['indent(', scope.lookup('x'), ', ', "scope.lookup('indentprefix'))"]))
        I('POP_SCOPE')
        I('RETURN')
        LABEL('Call')
        I('PUSH_SCOPE')
