...
class Assembler(Grammar):
    ...
    labels = {'Grammar': 0, 0: 5, 1: 12, 'Rule': 19, 'Label': 27, 'Target': 35, 'OpCode': 44, 'Value': 52, 'Action': 61, 'Set': 70, 'String': 87, 'List': 92, 'ListItem': 101, 'Format': 114, 'Indent': 123, 'Call': 132, 'Lookup': 145, 'asts': 154, 2: 156, 3: 163, 4: 171, 5: 173, 'astList': 177, 6: 179, 7: 186, 'ast': 193, 'py': 203}
    instructions = [
        PUSH_SCOPE,
        MATCH_ANY,
        BIND,
        ...
        'ys',
        ACTION,
        lambda self: self.bind('code', self.lookup('list')(), lambda: self.bind('labels', self.lookup('dict')(), lambda: self.bind('', self.lookup('ys'), lambda: join(['class ', self.lookup('x'), '(Grammar):\n\n', indent(join(['labels = ', self.lookup('repr')(self.lookup('labels')), '\n', 'instructions = [\n', indent(join([self.lookup('join')(self.lookup('code'), ',\n')]), self.lookup('indentprefix')), '\n]\n']), self.lookup('indentprefix'))])))),
        POP_SCOPE,
        RETURN,
        PUSH_SCOPE,
        MATCH_ANY,
